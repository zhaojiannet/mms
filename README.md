...
2. 在 1Panel "网站" 给**每个商户**建一个反代站点（`<slug>.vip.zhaojian.net`，DNS 手动加 A 记录），站点配置内按路径分流。`proxy_set_header Host $host` 一行都不能少：后端靠 Host 从子域解析租户，传丢了整站 404，运营后台的主机名校验也会失效：
   ...
   反代落在本机回环，但后端在容器里：nginx 连到映射端口时经过 Docker NAT，容器看到的来源地址是网桥网关（`172.x.0.1`），不是 `127.0.0.1`。所以 `.env` 必须配 `TRUSTED_PROXIES=172.16.0.0/12,127.0.0.1/32`——只配回环等于没配，`ClientIP` 拿到的全是网桥 IP：审计日志记不到真实来源，per-IP 限流把所有访客算作同一个来源，任何人打满登录限额，全站一分钟内都登不进去。
