...
const countCardUsage = `-- name: CountCardUsage :one
SELECT count(*) FROM transactions WHERE card_id = $1
`

func (q *Queries) CountCardUsage(ctx context.Context, cardID pgtype.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, countCardUsage, cardID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
