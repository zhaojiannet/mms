<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between gap-2 flex-wrap">
      ...
      <UButton icon="i-lucide-user-plus" @click="openCreate">新增员工</UButton>
    </div>
      ...
      <table class="w-full text-base">
        <thead class="bg-stone-50/60 dark:bg-stone-950/40 text-stone-500 text-xs tracking-wide">
          <tr>
            ...
            <th class="text-center px-4 py-3 font-medium">提成</th>
        ...
        <tbody class="divide-y divide-stone-200/60 dark:divide-stone-800">
          <tr v-for="s in items" :key="s.id" class="hover:bg-stone-50/60 dark:hover:bg-stone-800/40">
            ...
            <td class="px-4 py-3 text-center text-sm tabular-nums">
              ...
              <UBadge v-else label="不计" color="neutral" variant="soft" size="sm" />
            </td>
    ...
    <div v-else-if="!loading" class="py-16 text-center text-stone-500">暂无员工</div>
    <div v-if="loading" class="space-y-2"><USkeleton v-for="i in 3" :key="i" class="h-14 rounded-2xl" /></div>

    <USlideover v-model:open="formOpen" :title="editingId ? '编辑员工' : '新增员工'" :ui="{ content: 'w-full sm:max-w-md' }">
      <template #body>
        <UForm :state="form" class="space-y-4" @submit="onSubmit">
          <UFormField label="姓名" required><UInput v-model="form.name" class="w-full" /></UFormField>
          <UFormField label="职位" required><UInput v-model="form.position" placeholder="发型师 / 美容师 / 前台" class="w-full" /></UFormField>
          <UFormField label="手机"><UInput v-model="form.phone" class="w-full" /></UFormField>
          <UFormField label="入职日期"><UInput v-model="form.hire_date" type="date" class="w-full" /></UFormField>
          <UCheckbox v-model="form.counts_commission" label="参与提成核算" />
          <UFormField v-if="form.counts_commission" label="默认提成率（0-1）">
            <UInput v-model="form.default_commission_rate" type="number" step="0.001" min="0" max="1" class="w-full" />
          </UFormField>
...
interface Staff {
  ...
  hire_date: string | null; status: 'active' | 'inactive'
...
function resetForm() {
  form.name = ''; form.position = ''; form.phone = ''; form.hire_date = ''
...
function openEdit(s: Staff) {
  ...
  form.phone = s.phone ?? ''; form.hire_date = s.hire_date ?? ''
