      ...
      <table class="w-full text-base">
        <thead class="bg-stone-50/60 dark:bg-stone-950/40 text-stone-500 text-xs tracking-wide">
          <tr>
            ...
            <th class="text-center px-4 py-3 font-medium">折扣率</th>
            <th class="text-center px-4 py-3 font-medium">折扣显示</th>
        ...
        <tbody class="divide-y divide-stone-200/60 dark:divide-stone-800">
          <tr v-for="c in items" :key="c.id" class="hover:bg-stone-50/60 dark:hover:bg-stone-800/40">
            ...
            <td class="px-4 py-3 text-center text-stone-600 tabular-nums">{{ c.discount_rate }}</td>
            ...
            <td class="px-4 py-3 text-right">
              <UButton size="xs" variant="ghost" color="neutral" @click="openEdit(c)">编辑</UButton>
              <UButton size="xs" variant="ghost" color="error" @click="confirmDelete(c)">删除</UButton>
            </td>
          </tr>
        </tbody>
      </table>
    </div>
    <div v-else-if="!loading" class="py-16 text-center text-stone-500">暂无卡型</div>
    <div v-if="loading" class="space-y-2"><USkeleton v-for="i in 3" :key="i" class="h-14 rounded-2xl" /></div>

    <USlideover v-model:open="formOpen" :title="editingId ? '编辑卡型' : '新建卡型'" :ui="{ content: 'w-full sm:max-w-md' }">
      <template #body>
        <UForm :state="form" class="space-y-4" @submit="onSubmit">
          <UFormField label="卡名" required><UInput v-model="form.name" placeholder="如：500 元储值卡" class="w-full" /></UFormField>
          <UFormField label="售价（元）" required><UInput v-model="form.price" type="number" step="0.01" class="w-full" /></UFormField>
          <UFormField label="折扣率（0-1，如 0.7 表示七折）" required>
            <UInput v-model="form.discount_rate" type="number" step="0.001" min="0.001" max="1" class="w-full" />
            <template #hint><span class="text-xs text-stone-500">当前：{{ displayRate(form.discount_rate) }}</span></template>
          </UFormField>
          <UFormField label="排序"><UInput v-model="form.sort_order" type="number" class="w-full" /></UFormField>
